...
async def level1(*, message: str, message_date: str) -> List[Category]:
    response = await Main.run_prompt_template(template=__prompt_template, replacers=__input_replacers, params=dict(message=message, message_date=message_date))
...
def level1_stream(*, message: str, message_date: str
    ...
    def run_prompt() -> typing.AsyncIterator[LLMResponse]:
        raw_stream = Main.run_prompt_template_stream(template=__prompt_template, replacers=__input_replacers, params=dict(message=message, message_date=message_date))
        return raw_stream
